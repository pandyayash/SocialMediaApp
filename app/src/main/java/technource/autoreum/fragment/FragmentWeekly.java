...
import technource.autoreum.adapter.AdptSettingInvoiceWeekly;
...
import technource.autoreum.interfaces.OndeleteNotification;
import technource.autoreum.model.MailBoxDBO;
...
public class FragmentWeekly extends Fragment {
    ...
    ArrayList<MailBoxDBO> weeklyArrayList;
    SwipeRefreshLayout pull_to_refresh;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_invoice, null, false);
        getViews(view);
    ...
    public void getViews(View rootView) {
        ...
        textView.setText("No weekly data Found");
            ...
            adptSettingInvoiceWeekly = new AdptSettingInvoiceWeekly(appContext, mailBoxDBOArrayList, new OndeleteNotification() {
                ...
                public void onClick(View view, int position, String type) {
                    if (!type.equalsIgnoreCase("invoice")) {
                        deleteNotification(weeklyArrayList.get(position).getId(), position);
                    }
                }
            });
                            ...
                            if (status.equalsIgnoreCase(Constants.SUCCESS)) {
                                ...
                                if (weekly.length() > 0) {
                                    for (int i = 0; i < weekly.length(); i++) {
                                        JSONObject weeklyData = weekly.getJSONObject(i);
                                        ...
                                        weeklyArrayList.add(mailbox);
                                    }

                                }
                                setData(weeklyArrayList);
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            String status = response.getString(Constants.STATUS);
                            if (status.equalsIgnoreCase(Constants.SUCCESS)) {
                                weeklyArrayList.remove(position);
