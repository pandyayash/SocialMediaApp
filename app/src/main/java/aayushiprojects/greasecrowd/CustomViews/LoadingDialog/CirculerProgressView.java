package aayushiprojects.greasecrowd.CustomViews.LoadingDialog;
...
public class CirculerProgressView extends View {
  ...
  public CirculerProgressView(Context context) {
    super(context);
    init(null, 0);
  }

  public CirculerProgressView(Context context, AttributeSet attrs) {
    super(context, attrs);
    init(attrs, 0);
  }

  public CirculerProgressView(Context context, AttributeSet attrs, int defStyle) {
  ...
  private void initAttributes(AttributeSet attrs, int defStyle) {
    ...
    int accentColor = getContext().getResources()
        .getIdentifier("colorAccent", "attr", getContext().getPackageName());
  ...
  protected void onDraw(Canvas canvas) {
    ...
    // Draw the arc
    float sweepAngle =
        (isInEditMode()) ? currentProgress / maxProgress * 360 : actualProgress / maxProgress * 360;
    if (!isIndeterminate) {
      canvas.drawArc(bounds, startAngle, sweepAngle, false, paint);
    } else {
      canvas.drawArc(bounds, startAngle + indeterminateRotateOffset, indeterminateSweep, false,
          paint);
    }
  }
  ...
  private AnimatorSet createIndeterminateAnimator(float step) {
    ...
    // Overall rotation
    ValueAnimator rotateAnimator1 = ValueAnimator
        .ofFloat(step * 720f / animSteps, (step + .5f) * 720f / animSteps);
    ...
    ValueAnimator backEndRetract = ValueAnimator
        .ofFloat(start, start + maxSweep - INDETERMINANT_MIN_SWEEP);
    ...
    ValueAnimator rotateAnimator2 = ValueAnimator
        .ofFloat((step + .5f) * 720f / animSteps, (step + 1) * 720f / animSteps);
